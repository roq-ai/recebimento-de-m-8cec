...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getGoods() {
    ...
    const response = await prisma.goods
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'goods'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createGoods() {
    ...
    const body = { ...req.body };
    if (body?.divergence?.length > 0) {
      const create_divergence = body.divergence;
      body.divergence = {
        create: create_divergence,
      };
    } else {
      delete body.divergence;
    }
