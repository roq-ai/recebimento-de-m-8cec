...
import { getBalances } from 'apiSdk/balances';
...
import { GoodsListPage as ShowCaseList } from 'pages/goods';

function DashboardPage() {
  const { session } = useSession();

  const balanceStat = useSWR<PaginatedInterface<BalanceInterface>>(
    () => `/balances`,
    () => getBalances({ limit: 1, offset: 0 }),
  );

  const goodsStat = useSWR<PaginatedInterface<GoodsInterface>>(
    () => `/goods`,
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Balances',
      result: balanceStat?.data?.totalCount,
      isLoading: balanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Goods',
      result: goodsStat?.data?.totalCount,
      isLoading: goodsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Inventories',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`goods`} passHref legacyBehavior>
                  <Button
