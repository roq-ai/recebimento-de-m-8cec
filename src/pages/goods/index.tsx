...
import { getGoods, deleteGoodsById } from 'apiSdk/goods';
import { GoodsInterface } from 'interfaces/goods';

type ColumnType = ColumnDef<GoodsInterface, unknown>;
...
export function GoodsListPage(props: GoodsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getGoods({
        relations: ['supplier', 'user', 'divergence.count', 'inventory.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<GoodsInterface>>(
  ...
  const handleView = (row: GoodsInterface) => {
    if (hasAccess('goods', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/goods/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('divergence', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'divergence',
          header: 'Divergence',
          accessorKey: 'divergence',
          cell: ({ row: { original: record } }: any) => record?._count?.divergence,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/goods/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('goods', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/goods/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('goods', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="goods" />
        </Flex>

        {hasAccess('goods', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/goods/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
